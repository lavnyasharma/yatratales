...
function ReviewForm({ packageId, packageName }: { packageId: string; packageName: string }) {
  ...
  const form = useForm<ReviewFormData>({
    ...
    defaultValues: {
      ...
      website: '',
      comment: '',
      // Don't set rating here - it will be set via selectedRating state
    },
  });

  // Sync selectedRating with form's rating field
  useEffect(() => {
    form.setValue('rating', selectedRating, { shouldValidate: true });
  }, [selectedRating, form]);
  ...
  const onSubmit = async (data: ReviewFormData) => {
    ...
    try {
      ...
      const docRef = await addDoc(collection(firestore, 'testimonials'), reviewData);
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    ...
                    >
                      <Star
                        className={`h-6 w-6 ${i < selectedRating
